/* 분석 7항목.
   ...
   가중치는 서버(crates/api/src/domain/analysis.rs)와 같은 값이어야 한다.
   갈리면 화면에 뜬 점수와 실제로 받은 배지가 달라진다 — 올린 사람 입장에서는
   그게 제일 나쁜 종류의 버그다.

   합이 정확히 100 이다. 서버 쪽에도 그걸 확인하는 테스트가 있다.

   여기 있는 건 무엇을 재는지 보여 주기 위한 표다. 점수는 서버가 매긴다.
   코드 품질은 메시 에셋에 해당이 없어 서버가 null 을 주고, 그 8%는 나머지에
   나뉜다 — 화면에서 0 으로 그리면 깎인 것처럼 읽힌다. */
