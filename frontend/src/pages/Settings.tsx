...
export function Settings() {
  ...
  if (auth.status === "loading") {
    return <main className="mx-auto max-w-[560px] px-5 py-16 text-xs text-faint">{t("불러오는 중")}</main>;
  }
  ...
  return (
    <main className="mx-auto max-w-[560px] px-5 py-12">
      ...
      <h1 className="mt-1 text-2xl font-bold text-ink">{t("설정")}</h1>

      <section className="mt-8">
        <h2 className="text-base font-bold text-ink">{t("내 정보")}</h2>
        <dl className="mt-4 border-t border-line">
          ...
          <div className="grid grid-cols-[104px_minmax(0,1fr)] gap-4 border-b border-line py-3">
            <dt className="text-xs text-faint">{t("계정 번호")}</dt>
      ...
      <section className="mt-10">
        <h2 className="text-base font-bold text-ink">{t("요금")}</h2>
        <p className="mt-2 text-xs text-muted">{t("구독과 크레딧 충전은 요금 화면에서 봅니다.")}</p>
        <Link
          to="/billing"
      ...
      <section className="mt-10">
        <h2 className="text-base font-bold text-ink">{t("보안")}</h2>
        <p className="mt-2 text-xs leading-relaxed text-muted">
          {t("비밀번호는 Argon2id 로 저장되며 원문은 어디에도 남지 않습니다. 세션은 서버가 들고 있어 로그아웃하면 그 쿠키가 그 자리에서 끊깁니다.")}
        </p>
        <p className="mt-3 text-[10px] text-faint">
          {t("비밀번호 변경과 계정 삭제는 아직 준비 중입니다.")}
        </p>
        <button
          type="button"
          onClick={() => void logOut()}
