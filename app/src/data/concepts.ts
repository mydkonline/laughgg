...
/* 활용형을 같이 적는다.
   ...
   한국어는 어간이 활용하면서 받침이 떨어진다. 사전형만 적어 두면 사람이
   실제로 쓰는 말이 안 걸린다 — "어두운 지하실" 이라고 써도 `어둡` 에
   안 맞는다(ㅂ 탈락). 화면에 "읽은 것" 을 띄우고 나서야 드러났다.

     어둡다 → 어두운    차갑다 → 차가운    거칠다 → 거친

   프롬프트를 쓸 때는 "어둡다" 가 아니라 "어두운" 이라고 쓴다. 규칙은
   사전이 아니라 사람이 치는 말에 맞춰야 한다. */
const RULES: [RegExp, Partial<Knobs>][] = [
  [/어둡|어두운|어두컴컴|음침|다크|dark|그림자|밤/, { tone: 80, sat: 36 }],
  [/밝|화사|낮|bright|햇|light/, { tone: 22, sat: 66 }],
  [/따뜻|웜|노을|황금|golden|warm/, { warm: 80 }],
  [/차갑|차가운|쿨|한기|얼음|서늘|cold|cool/, { warm: 18 }],
  [/금속|메탈|철|강철|metal|광택|반짝/, { gloss: 82 }],
  [/거칠|거친|무광|매트|낡|녹슨|rough|matte/, { gloss: 16 }],
